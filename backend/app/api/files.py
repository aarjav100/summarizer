...
def list_files(project_id: Optional[str] = None, db: Session = Depends(get_db)):
    try:
        ...
        db_files = db.query(FileItem).all()
        if db_files:
            results = []
            for f in db_files:
...
async def upload_file(
    ...
    try:
        ...
        db.commit()
        db_success = True
        print(f"File {filename} successfully parsed, chunked, and saved in Supabase database.")
    except Exception as e:
        print(f"Database write exception during upload: {e}")
        db.rollback()

    new_file = FileItemResponse(
        id=file_id,
        project_id=project_id,
        filename=filename,
        file_type=file_type,
    ...
    # Maintain mock fallback registry
